...
function FileUploader(props) {
  const { annexure, reference, docId, signed } = props;
  ...
  const obj = useContext(SplitViewContext);
  const isBM = obj?.tabIndex;
  ...
  const handleUploadEnclosure = (enclosureSubject, value, tempArr) => {
    ...
    if (annexure) {
      return props
        .uploadCorrespondenceAnnexure(docId, formData, enclosureSubject)
        .then((resp) => {
          // console.log(resp);
          try {
            if (resp.error) {
              ...
              setTimeout(() => {
                props.handleAdd(resp.response.annexure);
              });
              if (value) {
                handleClose();
              }
      ...
      return props
        .uploadCorrespondenceReference(docId, formData, enclosureSubject)
        .then((resp) => {
          // console.log(resp);
          try {
            if (resp.error) {
              ...
              setTimeout(() => {
                props.handleAdd(resp.response.references);
              });
              if (value) {
                handleClose();
              }
    ...
    <div>
      ...
      <form>
        <FileUpload
          ...
          notingSigned={props.notingSigned}
          upload={props.upload}
          disabled={signed}
        />
      </form>
      ...
      >
        ...
        >
          {annexure
            ? t("add_annexure_subject")
            : reference
            ? t("add_reference_subject")
