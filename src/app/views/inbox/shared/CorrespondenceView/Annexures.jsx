...
const Annexures = (props) => {
  ...
  const {
    ...
    rowId,
    setRowId,
    noteSigned, // allow changes to annexure only when note is unsigned
    openRepo,
    setOpenRepo,
    mergeDak,
  ...
  const [totalCount, setTotalCount] = useState(annexureList?.length);
  ...
  useEffect(() => {
    if (annexObj) {
      setIsPdf(annexObj?.fileName?.includes(".pdf"));
    }
  }, [annexObj]);

  const handleAddAnnex = (data) => {
    let tempArr = [];
    annexureList.map((item) => tempArr.push(item));
    data.map((item, i) =>
      tempArr.unshift({
        ...item,
    ...
    unstable_batchedUpdates(() => {
      setannexureList(tempArr);
      handleChange(tempArr[0]);
    });
  };

  const handleAnnex = (data) => {
    ...
    let tempArr = annexureList.map((item) =>
  ...
  useEffect(() => {
    if (
      !annexSigned &&
      annexObj?.uploader == dept &&
      annexObj?.fileName.includes(".doc") &&
      annexObj?.fileName.includes(".docx") &&
      !mergeDak
    ) {
      setShowEditor(true);
    } else {
  ...
  const handleChange = (item) => {
    if (item) {
      ...
      arr.length > 1
        ? setannexExtension(arr[arr.length - 1])
        : setannexExtension("docx");
      setannexSigned(item?.isSigned);
      setannexAnnoId(item?.annotationId);
      setannexUrl(item?.fileUrl);
  ...
  const handleDocumentRollback = async () => {
    ...
    let body = {
      ...
      reference: false,
      application: false,
      flagNumber: annexObj?.flagNumber,
    };
  ...
  const deleteEnclosureData = () => {
    setLoading(true);
    props
      .deleteCorrAnnexureData(rowId, annexObj?.flagNumber, annexObj?.fileId)
      .then((res) => {
        try {
          if (res.error) {
          ...
          setannexureList(res.response?.annexures);
          let newData = res.response?.annexures[0];
            ...
            >
              ...
              >
                {!outbox && !index && !preview && !mergeDak && (
                  <>
                    <Grid item>
                      {!annexSigned && annexObj?.uploader == dept ? (
                        <Tooltip
                        ...
                        >
                          <span>
                            <Button
                              ...
                              onClick={() => {
                                setFileId(annexObj.fileId);
                                setFlagNumber(annexObj.flagNumber);
                                setOpen(true);
                              }}
                              disabled={
                                isPdf ||
                        ...
                        >
                          <span>
                            <Button
                              ...
                              disabled={
                                isPdf || annexureList.length == 0 || noteSigned
                              }
                            >
                    ...
                    {/* <Grid item>
                      <Tooltip title={t("import_dak")} aria-label="Upload File">
                        <div>
                          <Button
                            id="FlagNumberEnclouser_sign_button"
                            variant="contained"
                            ...
                            onClick={() => {
                              setOpenRepo(true);
                              Cookies.set("destId", rowId);
                            }}
                            disabled={noteSigned}
                          >
                            <Backup style={{ fontSize: "1rem" }} />
                          </Button>
                        </div>
                      </Tooltip>
                    </Grid> */}
